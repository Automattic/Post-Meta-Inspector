	...
	$.fn.baldrick.registerhelper('baldrick_modal', {
		...
		request_complete  : function(obj, params){
			if(obj.params.trigger.data('modal')){
				var modal_id = 'wm', modal, modalBody;
				...
				modalBody.removeClass(obj.params.loadClass).show();
		...
		params  : function(params,defaults){
			...
			var trigger = params.trigger, modal_id = 'wm';
			...
			if(trigger.data('modal') && (params.url || trigger.data('modalContent'))){
				...
				body.addClass(params.loadClass);
...
jQuery(document).ready(function($){
	...
	// Baldrick Bindings
	$('.inspect-trigger').baldrick({
		request			:	ajaxurl,
		method			:	'POST',
		loadClass		:	'spinner loading',
		before			:	function(obj){
			$('.inspector-modal').empty();
		},
		helper			:	{
			bind		:	function(triggers){
				triggers.data('action', 'inspect_meta_modal');
			}
		}
	});
